...
+ (void)initialize
{
     checkedIcon     = [UIImage imageNamed:@"ic_item_selected"];
...
-(id)initWithFrame:(CGRect)frame{
    ...
    {
        _selectView=[[UIImageView alloc] initWithFrame:CGRectMake(frame.size.width-checkedIcon.size.width, 0, checkedIcon.size.width, checkedIcon.size.height)];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    float x=(self.tableView.frame.size.width-kThumbnailSize.width*columns-minimumInteritemSpacing*(columns-1))/2;
    [cell bind:tempAssets selectionFilter:picker.selectionFilter minimumInteritemSpacing:minimumInteritemSpacing minimumLineSpacing:minimumLineSpacing columns:columns assetViewX:x];
    return cell;
}
