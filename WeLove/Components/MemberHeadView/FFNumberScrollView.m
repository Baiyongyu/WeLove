...
@implementation FFNumberScrollView
...
- (void)stopAnimation{
    for(CALayer *layer in scrollLayers){
        [layer removeAnimationForKey:@"FFNumberScrollView"];
    }
}
...
- (void)createScrollLayers{
    CGFloat width = roundf(10); // 每个数字的宽度
...
- (void)createAnimations{
    ...
    for(CALayer *scrollLayer in scrollLayers){
        ...
        [scrollLayer addAnimation:animation forKey:@"FFNumberScrollView"];
